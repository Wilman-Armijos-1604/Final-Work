...
class InformacionCuentaTercero : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_informacion_cuenta_tercero)
        ...
        val btnGuardar = findViewById<Button>(R.id.btn_aceptarDatosCuentaTercero)
        val btnEliminar = findViewById<Button>(R.id.btn_eliminarCuentaTercero)

        val txtIdentification = findViewById<EditText>(R.id.txt_identificacionCuentaTercero)
        val txtOrganizacion = findViewById<EditText>(R.id.txt_organizacionCuentaTercero)
        val txtResponsable = findViewById<EditText>(R.id.txt_responsableCuentaTercero)
        val txtTipoCuenta = findViewById<AutoCompleteTextView>(R.id.txt_tipoCuentaTercero)
        ...
        db.collection("CuentasContables")
            ...
            .addOnSuccessListener { cuentasContablesFirestore ->
                Log.i(
                    "Ayuda",
                    "El tamaño del arreglo de Tipos es: ${cuentasContablesFirestore.size()}"
                )
            ...
            if (txtIdentification.text.toString() == "" || txtResponsable.text.toString() == "" || txtOrganizacion.text.toString() == "" || txtTipoCuenta.text.toString() == "") {
                ...
                if (banderaTipoCuenta) {
                    if (txtTipoCuenta.text.toString() != cuentaOpcion.nivel5) {
                        Toast.makeText(
                            this,
                            "Seleccione una opción del a lista de Tipo de Cuenta",
                            Toast.LENGTH_LONG
                        ).show()
                        banderaTipoCuenta = false
                    } else {
                        db.collection("CuentasContables")
